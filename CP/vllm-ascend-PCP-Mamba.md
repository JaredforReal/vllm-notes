...
## 附：SGLang GLM5Next 的 KDA CP 设计

KDA/Mamba/linear attention 依赖自然 token 顺序：causal conv1d 需要连续时间顺序，recurrent state/chunk scan 需要按序推进，state cache 的更新位置必须与 token 的真实顺序一致。zigzag 适合 attention query load balance，不适合作为 KDA 的执行顺序。因此 SGLang 把层间 residual stream 固定为 **plain layout**，由 KDA 层内部自行 all-gather full sequence：
