...
`vllm-ascend-zigzag.md`（序列切分）、`vllm-ascend-chunked-prefill-PCP.md`（chunked prefill 兼容）、
...
- **Chunked Prefill**：有 AllGatherQ / AllGatherKV / Ring 三种思路。GQA 后端实现 AllGatherQ（与 decode 流程一致），MLA 后端实现 AllGatherKV（与标准 prefill 一致），并按 workspace 分段处理以控峰值显存。详见 `vllm-ascend-chunked-prefill-PCP.md`。
